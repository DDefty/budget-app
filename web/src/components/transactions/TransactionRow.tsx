    ...
    return (
        <tr key={transaction.id}>
            ...
            <td className="p-4 text-sm foreground-light dark:text-slate-200">
                {transaction.category.name === 'Income' ? <span className="inline-flex items-center rounded-full bg-green-400/10 px-2.5 py-0.5 text-xs font-medium text-green-600"> {transaction.category.name} </span> : <span className="inline-flex items-center rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-medium text-primary"> {transaction.category.name} </span>}
            </td>
            <td className="p-4 text-sm foreground-light text-muted-light dark:text-slate-200">{transaction.account}	</td>
            {transaction.category.kind === "INCOME" ? <td className="p-4 text-sm foreground-light dark:text-slate-200 text-green-500">{transaction.amount} {transaction.currency}</td> : <td className="p-4 text-sm foreground-light dark:text-slate-200 text-red-500">- {transaction.amount} {transaction.currency}</td>}
